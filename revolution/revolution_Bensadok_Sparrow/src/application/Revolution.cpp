...
void Revolution::initSphere() {
  ...
  //  TODO
      ...
      for (int j=0;j<nbStack;j++){
          ...
          p.push_back(cos(teta)*sin(phi));
          p.push_back(cos(phi));
          p.push_back(sin(teta)*sin(phi));
          teta += ((M_PI*2)/(nbSlice-1));
  ...
  // *******
    for (int j=0;j<nbStack-1;j++){
        for (int i=0;i<nbSlice-1;i++){
          index.push_back(j*(nbSlice)+i);
          index.push_back(j*(nbSlice)+i+1);
          index.push_back((j+1)*(nbSlice)+i);


          index.push_back((j+1)*(nbSlice)+i+1);
          index.push_back((j+1)*(nbSlice)+i);
          index.push_back(j*(nbSlice)+i+1);
      }
  }
//cout <<p.size()<< endl;
//cout <<nbSlice*nbStack << endl;
...
void Revolution::initRevolution() {
  ...
  for (int j=0;j<nbStack;j++){
      float teta=0;
      for (int i=0;i<nbSlice;i++){
        p.push_back((_profile[j].rotationY(teta).x()));
        p.push_back((_profile[j].rotationY(teta).y()));
        p.push_back((_profile[j].rotationY(teta).z()));
        teta += ((M_PI*2)/(nbSlice-1));
  ...
  Vector3 dir,dir2;
  Vector3 orth,orth2,moy;
  for (int j=0;j<nbStack;j++){
      float teta=0;
      for (int i=0;i<nbSlice;i++){
            dir=_profile[j+1]-_profile[j];
            dir2=_profile[j+2]-_profile[j+1];

            orth = Vector3(-dir.y(),dir.x(),0);
            orth2 = Vector3(-dir2.y(),dir2.x(),0);

            moy.mid(orth2,orth);
            n.push_back(moy.rotationY(teta).x());
            n.push_back(moy.rotationY(teta).y());
            n.push_back(moy.rotationY(teta).z());
            teta += ((M_PI*2)/(nbSlice-1));
    }
