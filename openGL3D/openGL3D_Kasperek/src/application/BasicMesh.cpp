...
void BasicMesh::initObj(const ObjLoader &obj) {
  ...
  for(unsigned int i=0;i<obj.nbFace();++i) {
    for(unsigned int j=0;j<3;++j) {
        _attribute.push_back((float)obj.positionVertex(i,j).x());
        _attribute.push_back((float)obj.positionVertex(i,j).y());
        _attribute.push_back((float)obj.positionVertex(i,j).z());
...
void BasicMesh::draw() {
  ...
  //glDrawElements(GL_TRIANGLES,_element.size(),GL_UNSIGNED_INT,(const GLvoid *)(0*sizeof(GLuint)));
    glDrawArrays(GL_TRIANGLES,0,_attribute.size()-3);
  glBindVertexArray(0);
}
